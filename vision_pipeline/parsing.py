...
def parse_declarations(lines: list[ParsedLine]) -> dict[str, dict[str, Any]]:
    ...
    for index, line in enumerate(lines):
        ...
        _set_if_found(result, "mrp", _money_after_label(normalized, r"(?:m\.?(?:r\.?)?p\.?|maximum retail price)"), line.confidence)
        _set_if_found(result, "net_quantity", _quantity_after_label(normalized), line.confidence)
        _set_if_found(result, "manufacturing_date", _date_after_label(normalized, r"(?:mfg|mfd|manufactur(?:ed|ing)|packed)(?:\s*date)?"), line.confidence)
        _set_if_found(result, "expiry_date", _date_after_label(normalized, r"(?:exp(?:iry)?|use\s*by|best\s*before)(?:\s*date)?"), line.confidence)
        _set_if_found(result, "unit_sale_price", _money_after_label(normalized, r"(?:unit\s*(?:sale\s*)?price|price\s*per\s*(?:kg|g|l|ml))"), line.confidence)

        contact = _contact_after_label(normalized)
        if contact:
            _set_if_found(result, "consumer_care_contact", contact, line.confidence)
        origin = re.search(r"(?:country\s*of\s*origin|made\s*in)\s*[:\-]?\s*([A-Za-z ]{3,})", normalized, re.I)
        if origin:
            _set_if_found(result, "country_of_origin", origin.group(1).strip(" ."), line.confidence)
        ...
        if re.search(r"\b(?:manufactured\s+by|manufactured\s*&\s*marketed\s+by|marketed\s+by|packed\s+by)\b", lowered):
            ...
            if value:
                _set_if_found(result, "manufacturer_name", value, line.confidence)
            next_line = lines[index + 1] if index + 1 < len(lines) else None
            if next_line and _looks_like_address(next_line.text):
                _set_if_found(result, "manufacturer_address", next_line.text.strip(), next_line.confidence)
        elif re.search(r"\b(?:address|add\.)\b", lowered):
            address = re.split(r"\b(?:address|add\.)\b\s*[:\-]?", normalized, flags=re.I)[-1].strip()
            _set_if_found(result, "manufacturer_address", address, line.confidence)
    return result
...
def _set_if_found(result: dict[str, dict[str, Any]], field: str, value: str | None, confidence: float) -> None:
    if value and not result[field]["present"]:
